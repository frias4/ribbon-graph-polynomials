## Script to draw a ribbon graph and compute its ribbon graph polynomial (Bollobás-Riordan)
## By José Frías
...
def draw_canvas(g,v):
    ...
    pts = []
    plt.title('Ribbon Graph Polynomial', fontsize=16)
...
# The function 'draw_graph()' the canvas (polygon and 'fat' vertices of the graph) to plot the
...
# The function 'components' counts the number of components of the state graph A
def components(A,v):
    comp = np.full(v, -1)
    a=int(A.shape[0]/2)
    index=0
    for i in range(a):
        if comp[A[i][0]] ==-1 and comp[A[i+a][0]] ==-1:
            comp[A[i][0]] = index
            comp[A[i+a][0]]  = index
            index +=1
        elif comp[A[i][0]] ==-1 and comp[A[i+a][0]] >-1:
            comp[A[i][0]] = comp[A[i+a][0]]
        elif comp[A[i][0]] >-1 and comp[A[i+a][0]] ==-1:
            comp[A[i+a][0]] = comp[A[i][0]]
        else:
            ma = max(comp[A[i][0]],comp[A[i+a][0]])
            mi = min(comp[A[i][0]],comp[A[i+a][0]])
            comp[comp == ma ] = mi
    m = comp[comp==-1].shape[0]
    n= len(set(comp[comp>-1].tolist()))
    return m+n

# The auxiliary function 'next'
def next(A, v):
    B=A[A[:,0]==v[0]]
    aux = B[:,1]
    if v[1] == max(aux):
...
# The function 'faces' counts the number of boundary components of ribbon graph A
def faces(A,v):
    a=int(A.shape[0]/2)
    ref= np.full(2*a,0)
    count = 0
    while 0 in ref or 1 in ref or -1 in ref:
        ...
        if ref[first] == 1:
            ...
            if ref[first] == -1:
                ...
                ref[first] = 1
        while np.array_equal(A[nex] ,A[first])==False or (np.array_equal(A[nex] ,A[first])==True and id ==-ref[first]):  #id==-1): ###OJJJO
            if id == 1:
                ...
                id = 1
        count +=1
    return(v-len(set(A[:,0].tolist()))+ count)

# The function 'RGpolynomial_state(A,v)' computes the contribution to the ribbon graph polynomial
# of graph stae state 'A'.
def RGpolynomial_state(A,v):
    x, y, z = symbols('x,y,z')
    e=int(A.shape[0]/2)
    k=components(A,v)
    f= faces(A,v)
    ...
    n=e-r
    rg=v-1
    B = ((x-1)**(rg-r)) * (y**n)*(z**(k-f+n))
    return B

# 'RGpolynomial(aristas)' is the main function of this module and computes the ribbon graph polynomial
# of an embedded graph in an orientable surface presented in the output format of module 'embedded_graph_canvas.py'
def RGpolynomial(aristas):
    x, y, z = symbols('x,y,z')
    P = 0
    a = int(len(aristas[:,0])/2)
    for i in it.product([0, 1], repeat = a):
        subgraph = aristas[np.concatenate((np.asarray(i),np.asarray(i)), axis=0)==1]
        P = P +  RGpolynomial_state(subgraph,v)
    return P
...
print('The ribbon graph polynomial of the provided graph is', P)
